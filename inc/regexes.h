...
regex labelAndOptionalCommand(beginLineRegStr + "(" + symbolRegStr + "):(.*)" + endLineRegStr); // something is after label
...
// jumps
regex jumpInstructionsReg("^(call|jmp|jeq|jne|jgt) (.*)$");
// load/store
regex twoOperandInstructionOneRegisterOtherAnyAddrMode("^(ldr|str) (r[0-7]|psw),(.*)$");


string registerAddrModeDataRegStr = "(" + registerRegStr + ")";
string registerIndAddrModeDataRegStr = R"reg(\[)reg" + spacesOrTabsRegStr + "(" + registerRegStr + ")" + spacesOrTabsRegStr + R"reg(\])reg";
string registerIndWithDisplacementAddrModeDataRegStr = R"reg(\[)reg" + spacesOrTabsRegStr + "(" + registerRegStr + ")" + spacesOrTabsRegStr +
...
regex jmpInstructionRegisterIndWithDisplacementsAddrReg(R"reg(^\*)reg" + registerIndWithDisplacementAddrModeDataRegStr + "$");
